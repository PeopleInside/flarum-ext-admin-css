    ...
    (new Extend\Frontend('admin'))
        ...
        ->content(function (Document $document, Request $request) {
            ...
            if (!empty($customCss)) {
                ...
                // 1. Previene la chiusura prematura del tag <style>
                ...
                // 3. Neutralizza URL javascript: (case-insensitive, gestisce spazi)
                $safeCss = preg_replace('/javascript\s*:/i', 'blocked:', $safeCss);
                // 4. Neutralizza expression() (vettore legacy IE)
                $safeCss = preg_replace('/expression\s*\(/i', 'blocked(', $safeCss);
                // 5. Neutralizza behavior: (vettore legacy IE)
